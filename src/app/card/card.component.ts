  ...
  template: `
    <div class="card">
      ...
      <div class="card-body">
        <h5 class="card-title">
          {{ product.productName }}
          <sup [style.opacity]="0.5">{{ product.productCode }}</sup>
        </h5>
        ...
        <p>
          Наличие:
          {{
            (product.count > 0 && product.count + ' шт.') ||
              'отсутствует на складе'
          }}
        </p>
        <app-button (click)="handleAddCart(product)" [color]="'primary'"
